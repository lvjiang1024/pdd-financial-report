...
数据本体和口径说明存档在 `data/cash_flow_quality.json`，同样是手写死在
`report_template.html` 里的 `CFQ` 常量，不是自动从季度流水线生成的。

PDD 是美股/SEC 申报主体，不适用 A 股年报那套"收现比"（销售商品、提供劳务收到的现金 / 营业收入）
指标——US GAAP 现金流量表用间接法，不单独披露这笔现金收款。改用了三个等价可得的角度：

- **净现比**（OCF / 归母净利润）：核心的"利润有没有变成真金白银"指标，直接可算，不需要额外抓数据。
- **"应付商家"浮存金对 OCF 的贡献占比**：PDD 作为撮合平台，买家付款在结算给商家前会先计入
  "应付商家"负债，这笔负债的年度增量会直接体现为经营现金流的增加——增量越大，说明当年 OCF
  里有多少其实是平台规模扩张带来的"浮存金"效应，而不是纯利润转化。2019-2020 年这个占比高达
  85%，2022 年后降到 2%-15%，是 PDD 现金流质量分析里最有信息量的一个发现。
- **有息负债**（可转换债券余额，PDD 没有银行借款）：反映公司是否依赖举债运转，PDD 在
  2019-2020 年因发债规模上升，此后逐年偿还，2025 年底可转债余额归零。

资本开支（capex）、可转换债券余额、期末现金余额（cash and cash equivalents，窄口径，
不含受限现金和短期投资）这三项，6-K 季度公告都不披露，只能去 **SEC 20-F 年报**的完整现金流量表
和资产负债表附注里找，用了哪几份 20-F 见 `cash_flow_quality.json` 的 `20f_accessions_used`。

新增一年数据时：营收/净利润/OCF 走 `pipeline/` 常规流程即可拿到；capex、可转债余额、
期末现金、应付商家余额这四项要等公司发布**当年年度 20-F**（通常次年 4 月）后手动去查，
在此之前只能先把新一年的 OCF/净利润更新上去，现金流质量 Tab 的其余字段留空或沿用上一年年报的口径说明。
